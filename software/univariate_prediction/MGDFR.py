...
class Mask:
    def __init__(self, N_x, gamma, seed=0):
        np.random.seed(seed)
        self.Win = gamma*(-1)**np.random.randint(0,2,(N_x))
    ...
    for i in range(4):
        ...
        if(not os.path.isfile(f'./result/{dataset}/dataset.npz')):
            try:
                ...
                pass
            for year in [2013, 2014, 2021, 2022]:
                try:
                    os.mkdir(f'./result/DST_{str(year)}')
                except:
                    pass
    
                dynamics = DST(year)
                u, d, T, eval_func = dynamics.generate_data()
                np.savez(f'./result/DST_{str(year)}/dataset', u=u, d=d, T=T, eval_func=eval_func)
